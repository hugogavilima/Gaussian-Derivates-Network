...
def initWeights(input_channels, output_channels, num_basis, order, separated):
    ...
    if separated:
        ...
        weights = tensorflow.Variable(initial_value = tensorflow.random.uniform(shape = (int(num_basis), int(input_channels),   int(output_channels/num_basis), int(numFiltersPerBasis)), minval = -5e-1, maxval = 5e-1, dtype = 'float'), name = 'clWeights', trainable = True)
